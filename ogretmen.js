...
$("taskForm").addEventListener("submit",async e=>{e.preventDefault();const sid=$("taskStudent").value;const s=students.find(x=>x.id===sid);if(!s)return;
 ...
 msg("taskMsg","Görev kaydedildi, bildirim gönderiliyor…");
 const bildir=httpsCallable(functions,"gorevBildirimiGonder");
 try{
  const sonuc=await bildir({taskId:gorevRef.id});
  const ogrenci=sonuc.data?.studentResult||{};
  const veli=sonuc.data?.parentResult||{};
  const toplam=Number(ogrenci.successCount||0)+Number(veli.successCount||0);
  const hata=Number(ogrenci.failureCount||0)+Number(veli.failureCount||0);
  msg("taskMsg",toplam>0
    ?`Görev kaydedildi. Bildirim ${toplam} cihaza gönderildi${hata?` • ${hata} hata`:""}.`
    :"Görev kaydedildi fakat aktif bildirim cihazı bulunamadı.","success");
 }catch(bildirimHatasi){
  console.error("Bildirim gönderme hatası:",bildirimHatasi);
  msg("taskMsg","Görev kaydedildi ancak bildirim gönderilemedi: "+(bildirimHatasi.message||"Bilinmeyen hata"),"error");
 }
 e.target.reset();$("taskOrder").value=1;$("taskApproval").checked=true;await loadTasks();renderStats()}
